...
class default_conv_layer(object):
    def __init__(self, input_x, in_channel, out_channel,
        ...
        :param data_format: channel last or second
        """
        ...
        with tf.variable_scope('conv_layer_{0}'.format(index)):
            with tf.name_scope('conv_kernel'):
                w_shape = [kernel_shape, kernel_shape, in_channel, out_channel]
                weight = tf.get_variable(
                    name='conv_kernel_{0}'.format(index),
                    shape=w_shape,
            ...
            with tf.variable_scope('conv_bias'):
                ...
                bias = tf.get_variable(
                    name='conv_bias_{0}'.format(index), shape=b_shape,
                    initializer=tf.glorot_uniform_initializer(seed=rand_seed))
                self.bias = bias
            ...
            cell_biased = tf.nn.bias_add(conv_out, self.bias,
                                         data_format=data_format)
            ...
            tf.summary.histogram(
                'conv_layer/{}/kernel'.format(index),
                weight
            )
            tf.summary.histogram(
                'conv_layer/{}/bias'.format(index),
                bias
            )
            tf.summary.histogram(
                'conv_layer/{}/activation'.format(index),
                cell_out
            )
...
class fc_layer(object):
    def __init__(self, input_x, in_size, out_size, rand_seed,
                 activation_function=None, index=0):
        """
        ...
        """
        with tf.variable_scope('fc_layer_{0}'.format(index)):
            with tf.name_scope('fc_kernel'):
                w_shape = [in_size, out_size]
                weight = tf.get_variable(
                    name='fc_kernel_{0}'.format(index),
                    shape=w_shape,
                    initializer=tf.glorot_uniform_initializer(seed=rand_seed))
                self.weight = weight
...
def _calculate_fan_in_and_fan_out(in_channel, out_channel, kernel_size):
    """
    The way to calculate fan in and fan out in pytorch.

    :param in_channel: number of input channels
    :param out_channel: number of output channels
    :param kernel_size: the size of the kernel
    :return: the fan in and fan out
    """
    num_input_fmaps = in_channel
    num_output_fmaps = out_channel
...
class spectral_conv_layer(object):
    def __init__(self, input_x, in_channel, out_channel,
                 ...
                 is_relu=True, strides=(1, 1, 1, 1), init_type="standard"):
        """
        ...
        def _glorot_sample(kernel_size, n_in, n_out):
            # TODO: consider using the _calculate_fan_in_and_fan_out from pytorch
            if init_type == "pytorch":
                n_in, n_out = _calculate_fan_in_and_fan_out(n_in, n_out,
                                                            kernel_size)
        ...
        with tf.variable_scope('spec_conv_layer_{0}'.format(index)):
            ...
            self.cell_out = tf.nn.bias_add(conv_out, bias,
                                           data_format=data_format)

            if is_relu:
                self.cell_out = tf.nn.relu(self.cell_out)
