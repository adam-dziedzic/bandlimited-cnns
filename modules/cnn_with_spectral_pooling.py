...
class CNN_Spectral_Pool(object):
    ...
    def __init__(self,
        ...
        self.M = M
        self.conv_filter_size = kernel_size
        self.gamma = gamma
        self.alpha = alpha
        self.beta = beta
    ...
    def get_nchannel_img_size(self, in_x):
        if self.data_format == "NCHW":
            _, nchannel, img_size, _ = in_x.get_shape().as_list()
        else:  # "NHWC"
            _, img_size, _, nchannel = in_x.get_shape().as_list()
        return nchannel, img_size

    def build_graph(self, input_x, input_y, train_phase, extra_conv_layer=True,
        ...
        for m in range(1, self.M + 1):
            ...
            nchannel, img_size = self.get_nchannel_img_size(in_x)
    ...
    def train(self, X_train, y_train, X_val, y_val,
              batch_size=512, num_epochs=10, val_test_frq=1,
        ...
        with tf.name_scope('inputs'):
            xs = tf.placeholder(shape=self.shape, dtype=tf.float32)
            ...
            for epc in range(num_epochs):
                ...
                for itr in range(iters):
                    ...
                    _, iter_loss, iter_eve = sess.run(
                        [step, loss, eve],  # eve - number of errors
                ...
                train_err = np.sum(train_eve_in_epoch) / len(y_train)
                ...
                print(results_str)
                with open(
                        "results/running_metrics_spectral_param_" +
                        "spectral_pooling_architecture_" + "_" + date_str,
                        'a') as f:
                    f.write(results_str + "\n")
                ...
                if (epc + 1) % val_test_frq == 0:
                    ...
                    if valid_acc > best_acc:
                        ...
                        saver.save(sess, model_dir + '/{}/{}'.format(
                            model_name,
                            full_model_name
                        ))
    ...
    def calc_test_accuracy(
        ...
        with tf.Session() as sess:
            ...
            print('ExperimentSpectralSpatial accuracy: {:.3f}'.format(test_acc))
