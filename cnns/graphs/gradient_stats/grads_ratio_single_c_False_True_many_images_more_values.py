...
for j, dataset in enumerate(datasets):
    ...
    org_grad = cols[org_col + 1][:limit]
    adv_grad = cols[adv_col + 1][:limit]
    print("col org: ", org_grad)
    print("col adv: ", adv_grad)
    print('col length: ', len(adv_grad))

    plt.plot(org_grad, adv_grad, label=f"{dataset[labels][0]}",
             # lw=line_width,
             color=colors[j],
...
plt.xlabel('$L_2$ of gradient for original image with correct class')
plt.ylabel('$L_2$ of gradient for adv image with adv class')
...
destination = dir_path + "/" + "grads_ratio_single_c_False_True_many_images_more_values_7." + format
