...
conv1D_x = [0, 1, 10, 25, 50, 75]
conv1D_y = [93.73, 77.36, 77.43, 77.52, 75.17, 61.8]
...
plots = [transform_svd, learn_svd_6_channels, learn_svd_3_channels, conv1D]
...
if transform_fft in plots:
    plt.plot(fft_preprocess_x, fft_preprocess_y,
             # label='fixed FFT compression (pre-processing only)',
             label='FFT',
             lw=lw,
             linestyle='--',
             marker='^',
...
if transform_svd in plots:
    plt.plot(svd_x, svd_y,
             # label='fixed SVD compression (pre-processing only)',
             label='compress inputs',
             linestyle='-',
             lw=lw,
             marker='o',
             color=get_color(MY_RED))
if learn_svd_6_channels in plots:
    plt.plot(train_on_svd_x, train_on_svd_y,
             # label='SVD representation (6 channels, U*D, V, n by p)',
             label='6 channels',
             lw=lw,
             linestyle=':',
             marker='^',
...
file_name = dir_path + "/" + "cifars_accuracy_test_svd_1D_maps"
