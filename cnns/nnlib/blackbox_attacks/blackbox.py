...
parser.add_argument('--iters', type=int, default=512, help='The number of iterations for iterative attacks')
parser.add_argument('--cw_conf', type=int, default=20, help='The confidence of adversarial examples for CW attack')
parser.add_argument('--spsa_samples', type=int, default=128,
...
def boundary_attack(target_model):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        inputs, targets = inputs.cpu().numpy()[0], int(targets.cpu().numpy())
        adversarial = attack(inputs.astype(np.float32), targets, iterations=args.iters, log_every_n_steps=999999)
        if adversarial is None:
            adversarial = inputs.astype(np.float32)
        if np.argmax(fmodel.predictions(adversarial)) == targets:
            correct += 1.
...
def spsa_attack(target_model):
    ...
    for epsilon in args.epsilons:
        spsa_params = {
            ...
            'clip_min': 0.0,
            'clip_max': 1.0,
            'nb_iter': args.iters,  # was 128 -> increased to 512
            'early_stop_loss_threshold': 0,
            'learning_rate': 0.1,  # default is 0.01
            'delta': 0.1,  # default is 0.01
