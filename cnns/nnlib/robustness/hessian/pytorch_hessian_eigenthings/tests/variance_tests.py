...
import numpy as np
import torch
from cnns.nnlib.robustness.hessian.pytorch_hessian_eigenthings.hessian_eigenthings import \
    compute_hessian_eigenthings
from cnns.nnlib.robustness.hessian.pytorch_hessian_eigenthings.tests.utils import \
    plot_eigenval_estimates, plot_eigenvec_errors
...
def get_full_hessian(loss_grad, model):
    ...
    for idx in range(l):
        grad2rd = torch.autograd.grad(g_vector[idx], model.parameters(),
                                      create_graph=True)
        cnt = 0
        for g in grad2rd:
...
def test_stochastic_hessian(model, criterion, real_hessian, x, y, bs=10,
                            ntrials=10):
    samples = [(x_i, y_i) for x_i, y_i in zip(x, y)]
    # full dataset
    ...
    # Plot eigenvalue error
    plt.suptitle(
        'Stochastic Hessian eigendecomposition errors: %d trials' % ntrials)
    plt.subplot(1, 2, 1)
    plt.title('Eigenvalues')
    plt.plot(list(range(nparams)), real_eigenvals, label='True Eigenvals')
    plot_eigenval_estimates(eigenvals, label='Estimates')
...
def test_fixed_mini(model, criterion, real_hessian, x, y, bs=10, ntrials=10):
    ...
    # Plot eigenvector L2 norm error
    plt.subplot(1, 2, 2)
    plt.title('Eigenvector cosine similarity')
    plot_eigenvec_errors(real_eigenvecs, eigenvecs, label='Estimates')
    plt.legend()
