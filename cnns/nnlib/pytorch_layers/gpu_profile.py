...
if 'GPU_DEBUG' in os.environ:
    ...
    info = f"profiling gpu usage to {gpu_profile_fn}"
...
def gpu_profile(frame, event, arg):
    ...
    if event == 'line':
        try:
            ...
            if 'gpu_memory_profiling' not in os.path.dirname(
                    os.path.abspath(filename)):
                lineno = None  # skip current line evaluation

            if ('TimeSeriesDatasets' in filename
