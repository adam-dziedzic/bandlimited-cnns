...
def make_layers(cfg, noise_init, noise_inner):
    layers = []
    in_channels = 3
    for i, v in enumerate(cfg):
        if v == 'M':
            ...
            if i == 0:
                ...
                noise_layer = Noise(noise_inner)
            layers += [noise_layer, conv2d, nn.BatchNorm2d(out_channels, affine=False), nn.ReLU()]
            in_channels = out_channels
    return nn.Sequential(*layers)

def stl10(n_channel, noise_init, noise_inner):
    cfg = [
        n_channel, 'M',
        2*n_channel, 'M',
        4*n_channel, 'M',
    ...
    layers = make_layers(cfg, noise_init, noise_inner)
