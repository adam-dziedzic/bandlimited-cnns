...
from .torch_01_range import to_01
from .torch_01_range import to_torch
...
class TestTorch01Range(unittest.TestCase):
    ...
    def test_cifar_to_01(self):
        dataset = 'cifar'
        x = torch.tensor(cifar10_example)
        y = to_01(x, dataset=dataset)
        print(y.min(), y.max())
        # assert y.min() >= 0.0
    ...
    def test_cifar_to_torch(self):
        dataset = 'cifar'
        x = torch.tensor(cifar10_example)
        y_01 = to_01(x, dataset=dataset)
        y2_01 = invert_normalization(x, dataset=dataset)
        y_torch = to_torch(y_01, dataset=dataset)
