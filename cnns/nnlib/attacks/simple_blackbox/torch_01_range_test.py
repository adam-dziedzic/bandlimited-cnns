...
from cnns.nnlib.attacks.simple_blackbox.utils import apply_normalization
from cnns.nnlib.attacks.simple_blackbox.utils import invert_normalization
...
class TestTorch01Range(unittest.TestCase):
    ...
    def test_cifar_to_01(self):
        ...
        assert torch.allclose(torch.min(torch.tensor([y.min().item(), 0.0])),
                              torch.tensor([0.0]),
                              atol=1e-05), f'{y.min()} is substantially lower than 0.0'
        # assert y.max() <= 1.0
        assert torch.allclose(torch.max(torch.tensor([y.max().item(), 1.0])),
                              torch.tensor([
                                               1.0])), f'{y.max()} is substantially greater than 1.0'
        y2 = invert_normalization(x, dataset=dataset)
    ...
    def test_imagenet_to_01(self):
        dataset = 'imagenet'
        ranger = Ranger(device=self.device)
        x = torch.tensor(imagenet_example).to(self.device)
        y = ranger.to_01(x, dataset=dataset)
        print(y.min(), y.max())
        # assert y.min() >= 0.0
        assert torch.allclose(torch.min(torch.tensor([y.min().item(), 0.0])),
    ...
    def test_imagenet_to_torch(self):
        dataset = 'imagenet'
        x = torch.tensor(imagenet_example).to(self.device)
        ranger = Ranger(device=self.device)
        y_01 = ranger.to_01(x, dataset=dataset)
