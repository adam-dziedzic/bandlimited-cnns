...
def batchnorm(x, name, phase, updates, gamma=0.96):
    k = x.get_shape()[1]
    runningmean = tf.get_variable(name + "/mean", shape=[1, k],
                                  initializer=tf.constant_initializer(0.0),
                                  trainable=False)
    runningvar = tf.get_variable(name + "/var", shape=[1, k],
    ...
    out = y * tf.get_variable(name + "/scaling", shape=[1, k],
                              initializer=tf.constant_initializer(1.0),
                              trainable=True) \
          + tf.get_variable(name + "/translation", shape=[1, k],
                            initializer=tf.constant_initializer(0.0),
                            trainable=True)
    return out
...
class _Function(object):
    def __init__(self, inputs, outputs, updates, givens, check_nan=False):
        assert all(len(i.op.inputs) == 0 for i in
                   inputs), "inputs should all be placeholders"
        self.inputs = inputs
        updates = updates or []
...
def mem_friendly_function(nondata_inputs, data_inputs, outputs, batch_size):
    if isinstance(outputs, list):
        return _MemFriendlyFunction(nondata_inputs, data_inputs, outputs,
                                    batch_size)
    else:
        f = _MemFriendlyFunction(nondata_inputs, data_inputs, [outputs],
                                 batch_size)
        return lambda *inputs: f(*inputs)[0]
...
class _MemFriendlyFunction(object):
    ...
    def __call__(self, *inputvals):
        ...
        for i_start in range(0, n, self.batch_size):
            slice_vals = [v[i_start:min(i_start + self.batch_size, n)] for v in
                          data_vals]
...
def lengths_to_mask(lengths_b, max_length):
    ...
    mask_bt = tf.expand_dims(tf.range(max_length), 0) < tf.expand_dims(
        lengths_b, 1)
    return mask_bt
